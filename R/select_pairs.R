...
select_pairs <- function(data_dir) {
  ...
  group_colors <- c(
    "#C7E9C0", "#C6DBEF", "#FCBBA1", "#FDD0A2", "#BCBDDC", "#D9D9D9", "#F6E8C3",
    "#DC143C", "#A1D99B", "#9ECAE1", "#FC9272", "#FDAE6B", "#9E9AC8", "#BDBDBD",
    "#DFC27D", "#F0EAD6", "#C3B091", "#007FFF", "#00FFFF", "#7FFFD4", "#228B22",
    "#808000", "#7FFF00", "#BFFF00", "#FFD700", "#DAA520", "#FF7F50", "#FA8072",
    "#FC0FC0", "#CC8899", "#E0B0FF", "#B57EDC", "#843179")

  ncolors <- length(group_colors)

  background <- paste0('url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA',
                       'oAAAAKCAYAAACNMs+9AAAAPklEQVQoU43Myw0AIAgEUbdAq7VADCQ',
                       'aPyww55dBKyQiHZkzBIwQLqQzCk9E4Ytc6KEPMnTBCG2YIYMVpHAC',
  ...
  server <- function(input, output, session) {
    ...
    pdata_r <- shiny::eventReactive(state$pdata, {
      ...
      for (rep_num in rep_nums) {
        ...
        pdata[rows, 'Replicate'] <<- paste('<div style="background-color:',
                                           color, ';"></div>')
      }
      ...
      if (paired) {
        ...
        for (pair_num in pair_nums) {
          ...
          pdata[rows, 'Pair'] <<- paste('<div style="background:', color,
                                        background, ';"></div>')
        }
      } else {
